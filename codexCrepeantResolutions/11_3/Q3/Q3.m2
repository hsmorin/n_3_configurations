-- Explicit double-octic derivation for Q3 from 11_3.m2
-- Run with: M2 --script Q3.m2
R = QQ[x0,x1,x2,x3,x4]
Q3 = x1^3*x4^2 - x1^3*x4 - 2*x1^2*x2*x3*x4 + x1^2*x2*x3 + 2*x1^2*x2*x4 - x1^2*x2 + x1^2*x3*x4 - x1^2*x4^3 - x1^2*x4^2 + x1^2*x4 + x1*x2^2*x3^2 - 2*x1*x2^2*x3 + x1*x2^2 - x1*x2*x3^2 + x1*x2*x3*x4^2 + 2*x1*x2*x3*x4 - 3*x1*x2*x4 + x1*x2 - x1*x3*x4 + x1*x4^3 - x2*x3^2*x4 + x2*x3^2 - x2*x3*x4^2 + 2*x2*x3*x4 - x2*x3
Q = Q3
F = homogenize(Q, x0)
S = QQ[x,y,z,w,u,v]
...
-- Selected chart: x1 = 1
-- P1 coordinate: x2 -> x
-- P3 coordinates: x0,x3,x4 -> y,z,w
-- Common u-power removed from the discriminant: 2
phi = map(S, R, {y,1,x,z,w})
g = phi F
T = terms g
